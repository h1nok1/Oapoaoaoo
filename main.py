...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .vk-tabs { display: flex; background: #fff; border-bottom: 1px solid #d3d9de; flex-shrink: 0; overflow-x: auto; }
        .vk-tabs button { flex: 1; padding: 12px 8px; background: transparent; border: none; border-bottom: 2px solid transparent; color: #828b95; font-size: 13px; font-weight: 500; cursor: pointer; transition: 0.2s; white-space: nowrap; min-width: 60px; }
        ...
        .modal-content { background: #fff; border-radius: 16px; padding: 32px; max-width: 360px; width: 90%; box-shadow: 0 20px 60px rgba(0,0,0,0.3); position: relative; max-height: 90vh; overflow-y: auto; }
        ...
        .story-row { display: flex; gap: 12px; padding: 12px 16px; background: #fff; border-bottom: 1px solid #edeef0; overflow-x: auto; }
        .story-item { display: flex; flex-direction: column; align-items: center; cursor: pointer; flex-shrink: 0; }
        .story-item .story-avatar { width: 64px; height: 64px; border-radius: 50%; border: 3px solid #4a76a8; padding: 2px; display: flex; align-items: center; justify-content: center; background: #fff; }
        .story-item .story-avatar img { width: 100%; height: 100%; border-radius: 50%; object-fit: cover; }
        .story-item .story-avatar .placeholder { font-size: 28px; }
        .story-item .story-name { font-size: 11px; color: #828b95; margin-top: 4px; max-width: 64px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .story-item .story-avatar.viewed { border-color: #828b95; opacity: 0.7; }
        .story-viewer { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: #000; z-index: 10000; display: flex; align-items: center; justify-content: center; flex-direction: column; }
        .story-viewer img, .story-viewer video { max-width: 100%; max-height: 80%; object-fit: contain; }
        .story-viewer .caption { color: #fff; font-size: 16px; padding: 16px; text-align: center; }
        .story-viewer .close { position: absolute; top: 20px; right: 20px; color: #fff; font-size: 32px; background: none; border: none; cursor: pointer; z-index: 10001; }
        .story-viewer .nav-left, .story-viewer .nav-right { position: absolute; top: 50%; transform: translateY(-50%); color: #fff; font-size: 40px; background: rgba(0,0,0,0.5); border: none; padding: 16px 8px; cursor: pointer; z-index: 10001; }
        .story-viewer .nav-left { left: 0; }
        .story-viewer .nav-right { right: 0; }
        .story-viewer .progress { position: absolute; top: 20px; left: 20%; right: 20%; height: 3px; background: rgba(255,255,255,0.3); border-radius: 2px; overflow: hidden; }
        .story-viewer .progress .fill { height: 100%; background: #fff; transition: width 0.1s linear; }
        .story-viewer .reactions-row { position: absolute; bottom: 100px; display: flex; gap: 12px; }
        .story-viewer .reactions-row button { background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); border-radius: 50%; width: 48px; height: 48px; font-size: 24px; cursor: pointer; transition: 0.2s; backdrop-filter: blur(5px); }
        .story-viewer .reactions-row button:hover { transform: scale(1.2); background: rgba(255,255,255,0.2); }
        .story-upload-btn { position: fixed; bottom: 80px; right: 20px; background: #4a76a8; color: white; border: none; border-radius: 50%; width: 56px; height: 56px; font-size: 28px; cursor: pointer; box-shadow: 0 4px 12px rgba(74,118,168,0.4); z-index: 100; }
        .story-upload-btn:hover { transform: scale(1.05); }
    </style>
</head>
<body>
<div id="app">
...
<script>
    ...
    async function loadStories() {
        try {
            ...
            if (usernames.length === 0) {
                container.innerHTML = `<div style="color:#828b95;font-size:13px;padding:4px 0;">Нет историй</div>`;
                return;
            }
    ...
    function showStory(username, index) {
        ...
        viewer.innerHTML = `
            ...
            ${isVideo ?
                `<video src="${story.media_url}" autoplay controls style="max-width:100%;max-height:70%;"></video>` :
            ...
            <div style="color:#fff;font-size:14px;margin-top:8px;">${index+1}/${stories.length}</div>
        `;
    ...
    async function reactToStory(reaction) {
        ...
        if (data.status === 'reacted' || data.status === 'updated') {
            ...
            toast.style.cssText = 'position:absolute;top:60px;color:#fff;background:rgba(0,0,0,0.6);padding:8px 16px;border-radius:20px;font-size:14px;';
    ...
    async function uploadStory() {
        ...
        input.onchange = async (e) => {
            ...
            const res = await fetch('/story/upload', {
                method: 'POST',
                body: formData
            });
            const data = await res.json();
