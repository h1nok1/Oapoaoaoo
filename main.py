...
async def upload_story(username: str = Form(...), file: UploadFile = File(...), caption: str = Form("")):
    ...
    c.execute("""INSERT INTO stories
              ...
              (story_id, username, media_type, file_path, caption[:200], datetime.now()))
    conn.commit()
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* АВТОРИЗАЦИЯ — чистая, без авто-входа */
        #authScreen {
            flex: 1;
            display: flex;
            flex-direction: column;
...
<script>
    ...
    async function createChannel() {
        ...
        if (data.channel_id) {
            alert('✅ Канал создан!');
            loadChannels();
            loadChats(); // обновляем список чатов (канал появляется и там)
        }
    }
