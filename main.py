...
OWNER_USERNAME = "seconddurov"
OWNER_PASSWORD = "020112"

c.execute("SELECT * FROM users WHERE username = ?", (OWNER_USERNAME,))
if not c.fetchone():
    pwd_hash, salt = hash_password(OWNER_PASSWORD)
...
<script>
    ...
    async function loadStories() {
        try {
            ...
            const data = await res.json();
            // Показываем истории в верхней части чатов
...
📞 Звонков: ${data.total_calls}`);
    ...
    function switchTab(tab) {
        ...
        if (targetId) {
            ...
            if (el) {
                ...
                if (tab === 'chats') {
                    loadChats();
                    // Добавляем строку историй в чаты
                    const chatContainer = document.getElementById('tab-chats');
                    if (chatContainer) {
                        let storyRow = document.getElementById('storyRow');
                        if (!storyRow) {
                            storyRow = document.createElement('div');
                            storyRow.id = 'storyRow';
                            storyRow.className = 'story-row';
                            storyRow.style.cssText = 'display:flex;gap:12px;padding:12px 16px;background:rgba(255,255,255,0.01);border-bottom:1px solid rgba(255,255,255,0.02);overflow-x:auto;flex-shrink:0;';
                            chatContainer.insertBefore(storyRow, chatContainer.firstChild);
                        }
                        loadStories();
                    }
                }
    ...
    // ---- СТИЛИ ДЛЯ ИСТОРИЙ (добавляем динамически) ----
