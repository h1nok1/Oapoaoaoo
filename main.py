...
<head>
    ...
    <style>
        /* ============================================================
           ПОЛНЫЙ НАБОР СТИЛЕЙ (Liquid Glass + Fsociety)
           ============================================================ */
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* ----- КНОПКИ СОЗДАНИЯ КАНАЛА И ГРУППЫ ----- */
        .action-buttons {
            display: flex;
            gap: 8px;
            padding: 8px 16px;
        ...
        <div id="tabContainer" style="flex:1;display:flex;flex-direction:column;overflow:hidden;">
            ...
            <div id="tab-chats" class="tab-content active" style="flex:1;display:flex;flex-direction:column;">
                ...
                <div class="action-buttons">
                    ...
                    <button class="group-btn" onclick="createGroup()">👥 Группа</button>
                </div>
...
<script>
    ...
    async function createGroup() {
        ...
        if (!title) return;
        const participantsInput = prompt('👤 Введите имена участников через запятую:');
        if (!participantsInput) return;
        const participants = participantsInput.split(',').map(s => s.trim()).filter(s => s);
        ...
        try {
            const res = await fetch('/create_group', {
                method: 'POST',
                headers: getVKHeaders(),
                body: JSON.stringify({title, participants: participants.join(',')})
            });
            const data = await res.json();
            if (data.chat_id) {
    ...
    async function renderProfile() {
        ...
        const isOwnerLocal = currentUser === 'seconddurov' || data.role === 'owner';
        ...
        container.innerHTML = `
            <div style="text-align:center;">
                ...
                <div class="profile-name">
                    ...
                    <span class="sub">${roleDisplay} • ${data.phone || ''}</span>
                </div>
