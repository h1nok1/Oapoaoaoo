...
if not c.fetchone():
    ...
    c.execute("""INSERT INTO users
                 (id, username, password_hash, salt, premium, profile_emoji, role, nft_collection, bio, vk_id, phone)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
              (user_id, ADMIN_USERNAME, pwd_hash, salt, 1, "👑", "admin",
               json.dumps(["Корона Даркнета"]), "Владыка подполья", str(random.randint(100000000, 999999999)), "+1 343 438 5452"))
    conn.commit()
...
def encrypt_message_aes(key: bytes, plaintext: str) -> str:
    aesgcm = AESGCM(key)
    nonce = secrets.token_bytes(12)
    ciphertext = aesgcm.encrypt(nonce, plaintext.encode(), None)
    return base64.b64encode(nonce + ciphertext).decode()

def decrypt_message_aes(key: bytes, encrypted: str) -> str:
    ...
    try:
        while True:
            ...
            if event_type == "message":
                ...
                # Шифруем сообщение (E2E)
                encrypted_text = text
                # В реальном приложении здесь было бы шифрование на клиенте
                # Сервер просто сохраняет текст
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #profileContent::-webkit-scrollbar { width: 3px; }
        #profileContent::-webkit-scrollbar-thumb { background: rgba(255,26,26,0.06); border-radius: 4px; }
        .profile-avatar {
            width: 100px;
            height: 100px;
            border-radius: 50%;
...
<script>
    ...
    async function renderProfile() {
        ...
        const content = document.getElementById('profileContent') || document.createElement('div');
        ...
        content.innerHTML = `
            ...
            <input type="file" id="avatarInput" accept="image/*" style="display:none" onchange="uploadPhoto(event)">
    ...
    function switchTab(tab) {
        ...
        const tabs = { chats: 0, calls: 1, profile: 2, settings: 3 };
    ...
    async function showCallHistory() {
        ...
        const panel = document.getElementById('tab-calls') || document.createElement('div');
        panel.id = 'tab-calls';
        ...
        if (calls.length === 0) {
            panel.innerHTML = `<div style="text-align:center;padding:40px 20px;color:#553333;font-size:14px;">
    ...
    function showSettings() {
        const panel = document.getElementById('tab-settings') || document.createElement('div');
        panel.id = 'tab-settings';
        panel.className = 'tab-content';
        panel.style.padding = '16px';
        panel.style.overflowY = 'auto';
        panel.style.flex = '1';
        panel.innerHTML = `
            ...
            <div style="display:flex;flex-direction:column;gap:6px;">
                ...
                <div style="display:flex;justify-content:space-between;padding:10px 12px;background:rgba(255,26,26,0.02);border-radius:8px;border:1px solid rgba(255,26,26,0.02);">
                    ...
                    <span style="color:#ff1a1a;cursor:pointer;" onclick="loadAdminStats()">➜</span>
                </div>
            </div>
        `;
        
        const container = document.getElementById('mainScreen');
        const old = document.getElementById('tab-settings');
        if (old) old.remove();
        container.appendChild(panel);
        document.getElementById('chatList').style.display = 'none';
    }
