...
app = FastAPI(title="Nyx - Liquid Glass")
...
# ----- ТВОЙ АККАУНТ (создаётся в БД, но НЕ автовход) -----
...
# ----- ИСТОРИИ (ФИКС — СОХРАНЯЮТСЯ ПОСЛЕ ПЕРЕЗАХОДА) -----
...
@app.get("/story/reactions/{story_id}")
async def get_story_reactions(story_id: str):
    c.execute("""SELECT username, reaction, created_at
                 FROM story_reactions
                 WHERE story_id = ?
                 ORDER BY created_at DESC""", (story_id,))
    reactions = c.fetchall()
    return [{"username": r[0], "reaction": r[1], "time": r[2]} for r in reactions]

@app.get("/story/viewers/{story_id}")
async def get_story_viewers(story_id: str, username: str):
    c.execute("SELECT username FROM stories WHERE id = ?", (story_id,))
    story_owner = c.fetchone()
    if not story_owner or story_owner[0] != username:
        raise HTTPException(403, "Only story owner can see viewers")

    c.execute("""SELECT viewer_username, viewed_at
                 FROM story_views
                 WHERE story_id = ?
                 ORDER BY viewed_at DESC""", (story_id,))
    viewers = c.fetchall()
    return [{"username": v[0], "viewed_at": v[1]} for v in viewers]

@app.delete("/story/delete/{story_id}")
async def delete_story(story_id: str, username: str):
    c.execute("SELECT username FROM stories WHERE id = ?", (story_id,))
    story = c.fetchone()
    if not story:
        raise HTTPException(404, "Story not found")
    if story[0] != username:
        raise HTTPException(403, "You can only delete your own stories")

    c.execute("DELETE FROM stories WHERE id = ?", (story_id,))
    c.execute("DELETE FROM story_views WHERE story_id = ?", (story_id,))
    c.execute("DELETE FROM story_reactions WHERE story_id = ?", (story_id,))
    conn.commit()
...
# ----- ГЛАВНАЯ СТРАНИЦА (LIQUID GLASS + ПРОФИЛЬ + ЧИСТАЯ АВТОРИЗАЦИЯ) -----
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .glass-header .actions button:hover {
        ...
        .chat-item .avatar.premium {
            border-color: rgba(0, 255, 157, 0.2);
            box-shadow: 0 0 20px rgba(0, 255, 157, 0.04);
        }
        .chat-item .info {
            flex: 1;
            min-width: 0;
        ...
        .chat-header .back:hover { color: #00ff9d; }
        .chat-header .title {
            flex: 1;
            font-weight: 500;
            margin-left: 6px;
        ...
        .chat-header .actions button:hover { color: #00ff9d; }
        ...
        .msg .reactions span:hover { background: rgba(255, 255, 255, 0.08); }
        ...
        .msg-input-area button:hover { color: #00ff9d; }
        ...
        .channel-item .desc { font-size: 13px; color: #8899bb; }
        .channel-item .meta { font-size: 12px; color: #667788; margin-top: 4px; }
        ...
        .btn-primary { background: linear-gradient(135deg, #00ff9d, #00cc7d) !important; color: #0a0d14 !important; }
        .btn-primary:hover { box-shadow: 0 0 24px rgba(0, 255, 157, 0.15); }
        .btn-outline { border: 1px solid rgba(0, 255, 157, 0.15) !important; color: #00ff9d !important; }
        .btn-owner { border: 1px solid rgba(255, 255, 255, 0.06) !important; }
        ...
        .story-row::-webkit-scrollbar-thumb { background: rgba(0, 255, 157, 0.1); border-radius: 4px; }
        .story-item {
            display: flex;
            flex-direction: column;
            align-items: center;
        ...
        .story-item .story-avatar:hover { border-color: #00ff9d; box-shadow: 0 0 20px rgba(0, 255, 157, 0.1); }
        .story-item .story-avatar.viewed { border-color: #556677; opacity: 0.6; }
        .story-item .story-name { font-size: 10px; color: #8899bb; margin-top: 4px; max-width: 56px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        ...
        .story-viewer img, .story-viewer video { max-width: 100%; max-height: 80%; object-fit: contain; border-radius: 12px; }
        .story-viewer .caption { color: #e8edf5; font-size: 16px; padding: 16px; text-align: center; }
        .story-viewer .close {
            position: absolute;
            top: 20px;
            right: 20px;
        ...
        .story-viewer .close:hover { background: rgba(255, 255, 255, 0.08); }
        .story-viewer .nav-left, .story-viewer .nav-right {
            position: absolute;
            top: 50%;
        ...
        .story-viewer .nav-left:hover, .story-viewer .nav-right:hover { background: rgba(255, 255, 255, 0.06); }
        .story-viewer .nav-left { left: 8px; }
        .story-viewer .nav-right { right: 8px; }
        ...
        .story-viewer .progress .fill { height: 100%; background: #00ff9d; transition: width 0.1s linear; }
        .story-viewer .reactions-row {
            position: absolute;
            bottom: 80px;
            display: flex;
            gap: 12px;
        }
        .story-viewer .reactions-row button {
            background: rgba(255, 255, 255, 0.04);
            border: 1px solid rgba(255, 255, 255, 0.04);
            border-radius: 50%;
            width: 44px;
        ...
        .story-viewer .reactions-row button:hover { transform: scale(1.15); background: rgba(255, 255, 255, 0.08); }
        .story-upload-btn {
            position: fixed;
            bottom: 80px;
            right: 20px;
        ...
        .story-upload-btn:hover { transform: scale(1.05); box-shadow: 0 6px 32px rgba(0, 255, 157, 0.3); }
        ...
        .profile-name { font-size: 24px; font-weight: 600; text-align: center; margin-top: 12px; }
        .profile-emoji { font-size: 18px; text-align: center; color: #8899bb; }
        .profile-bio { font-size: 14px; color: #8899bb; text-align: center; margin: 8px 0; padding: 8px 12px; background: rgba(255, 255, 255, 0.02); border-radius: 8px; }
        .profile-nfts { display: flex; flex-wrap: wrap; gap: 6px; justify-content: center; margin: 8px 0; }
        .profile-nft { background: rgba(255, 255, 255, 0.02); padding: 4px 12px; border-radius: 12px; font-size: 13px; border: 1px solid rgba(255, 255, 255, 0.03); }
        ...
        .profile-actions .logout-btn {
            ...
            color: #ff4757;
            border-color: rgba(255, 71, 87, 0.05);
        }
    ...
    <div id="mainScreen">
        ...
        <div id="tab-channels" class="tab-content">
            <div style="padding:12px;">
                <button onclick="createChannel()" style="width:100%;padding:12px;background:linear-gradient(135deg,#00ff9d,#00cc7d);color:#0a0d14;border:none;border-radius:12px;font-size:15px;font-weight:600;cursor:pointer;font-family:inherit;">
                    ➕ Создать канал
                </button>
        ...
        <div id="tab-profile" class="tab-content">
            <div id="profileContent">
                ...
                <button onclick="document.getElementById('avatarInput').click()" style="display:block;margin:8px auto;padding:6px 16px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.04);border-radius:8px;color:#8899bb;cursor:pointer;font-size:13px;font-family:inherit;">
                    📷 Сменить аватар
                </button>
                <input type="file" id="avatarInput" accept="image/*" style="display:none" onchange="uploadProfileAvatar(event)">

                <div class="profile-name" id="profileUsername">username</div>
                <div class="profile-emoji" id="profileEmoji">𖤐</div>
                <div class="profile-bio" id="profileBioDisplay">—</div>
                <div style="margin:8px 0;">
                    <label style="font-size:12px;color:#667788;">📝 Изменить био</label>
                    <input id="profileBioInput" placeholder="Расскажи о себе..." style="width:100%;padding:8px 12px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.04);border-radius:8px;color:#e8edf5;font-size:14px;font-family:inherit;outline:none;margin-top:4px;">
                    <button onclick="updateBio()" style="margin-top:4px;padding:6px 16px;background:linear-gradient(135deg,#00ff9d,#00cc7d);color:#0a0d14;border:none;border-radius:8px;cursor:pointer;font-size:13px;font-weight:500;font-family:inherit;">Сохранить</button>
                </div>

                <div style="margin:8px 0;">
                    <div style="font-size:13px;color:#8899bb;margin-bottom:4px;">🎁 NFT-подарки</div>
                    <div class="profile-nfts" id="profileNfts"></div>
                </div>
...
<button id="storyUploadBtn" class="story-upload-btn hidden" onclick="uploadStory()">➕</button>
...
<script>
    ...
    async function login() {
        ...
        document.getElementById('storyUploadBtn').classList.remove('hidden');
        connectWebSocket();
        loadChats();
        loadChannels();
        loadStories();
    ...
    async function loadProfile() {
        ...
        document.getElementById('profileEmoji').textContent = data.emoji_status || '𖤐';
        document.getElementById('profileBioDisplay').textContent = data.bio || '—';
        ...
        const nftsContainer = document.getElementById('profileNfts');
        if (data.nfts && data.nfts.length > 0) {
            nftsContainer.innerHTML = data.nfts.map(n => `<span class="profile-nft">${n}</span>`).join('');
        } else {
            nftsContainer.innerHTML = '<span style="color:#667788;font-size:13px;">Нет подарков</span>';
        }
    }
    ...
    async function updateBio() {
        ...
        const res = await fetch('/update_bio', { method: 'POST', body: formData });
        const data = await res.json();
        if (data.status === 'ok') {
            alert('✅ Био обновлено!');
            loadProfile();
    ...
    async function loadStories() {
        try {
            ...
            if (usernames.length === 0) {
                container.innerHTML = `<div style="color:#667788;font-size:13px;padding:4px 0;">Нет историй</div>`;
                return;
            }
            ...
            container.innerHTML = usernames.map(uname => {
                ...
                return `
                    <div class="story-item" onclick="openStoryViewer('${uname}')">
                        ...
                        <div class="story-name">${uname}</div>
                    </div>
                `;
            }).join('');
        } catch(e) {
    ...
    async function openStoryViewer(username) {
        ...
        currentStoryIndex = 0;
        showStory(currentStoryUser, currentStoryIndex);
    }

    function showStory(username, index) {
        const stories = storiesData[username] || [];
        if (index >= stories.length || index < 0) { closeStoryViewer(); return; }
        ...
        viewer.innerHTML = `
            ...
            <div style="color:#8899bb;font-size:14px;margin-top:8px;">${index+1}/${stories.length}</div>
        `;
    ...
    async function reactToStory(reaction) {
        ...
        if (data.status === 'reacted' || data.status === 'updated') {
            ...
            toast.style.cssText = 'position:absolute;top:60px;color:#e8edf5;background:rgba(0,0,0,0.5);padding:8px 16px;border-radius:20px;font-size:14px;backdrop-filter:blur(8px);';
    ...
    async function loadChats() {
        try {
            ...
            if (chats.length === 0) {
                list.innerHTML = `<div style="text-align:center;padding:40px 20px;color:#667788;">
    ...
    async function loadChannels() {
        try {
            const res = await fetch(`/channels/all?username=${currentUser}`, { headers: getVKHeaders() });
            const channels = await res.json();
            const list = document.getElementById('channelList');

            if (channels.length === 0) {
                list.innerHTML = `<div style="text-align:center;padding:20px;color:#667788;">
            ...
            list.innerHTML = channels.map(ch => {
                ...
                <div class="channel-item">
                    <div style="display:flex;justify-content:space-between;align-items:center;">
                        ...
                        ${isOwner ? '<span style="font-size:11px;color:#00ff9d;">👑 Владелец</span>' : ''}
                    </div>
                    ...
                    <div class="actions">
                        ...
                        <button class="btn-outline" onclick="subscribeChannel('${ch.channel_id}')">${ch.is_subscribed ? '✅ Подписан' : 'Подписаться'}</button>
    ...
    function showReactionPicker(messageId) {
        ...
        picker.style.cssText = `
            ...
            background: rgba(20,28,43,0.8); backdrop-filter: blur(24px);
            -webkit-backdrop-filter: blur(24px);
        ...
        try {
            ...
            panel.innerHTML = `
                <div class="stats-grid">
                    ...
                    <div class="stat-card"><div class="value">${data.active_stories || 0}</div><div class="label">📸 Историй</div></div>
                </div>
                ...
                <div class="admin-section">
                    ...
                    ${users.map(u => `
                        <div class="admin-user-item">
                            ...
                            <span style="font-size:11px;color:#667788;">${u.messages} msgs</span>
                        </div>
                    `).join('')}
                </div>
            `;
    ...
    function switchTab(tab) {
        ...
        document.querySelectorAll('.glass-tabs button').forEach(el => el.classList.remove('active'));
        document.getElementById(`tab-${tab}`).classList.add('active');
        document.querySelector(`.glass-tabs button[data-tab="${tab}"]`).classList.add('active');
        if (tab === 'admin' && isAdmin) loadAdminStats();
        if (tab === 'channels') loadChannels();
        if (tab === 'chats') { loadChats(); loadStories(); }
