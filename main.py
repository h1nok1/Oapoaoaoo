...
<script>
    ...
    function switchTab(tab) {
        ...
        if (tab === 'calls') {
            alert('[+] Звонки доступны из чата (кнопка 📞)');
        }
        if (tab === 'profile') {
        ...
        if (tab === 'settings') {
            alert('[+] Настройки: версия 1.0 | Fsociety Edition');
        }
    }
        ...
        modal.innerHTML = `
            <div class="modal-content">
                ...
                <div style="text-align:center;">
                    ...
                    ${data.bio ? `<div style="font-size:13px;color:#884444;margin:8px 0;padding:8px;background:rgba(255,26,26,0.02);border-radius:8px;">${escapeHtml(data.bio)}</div>` : ''}
                    <button onclick="this.parentElement.parentElement.remove()" style="margin-top:12px;padding:8px 24px;background:rgba(255,26,26,0.02);color:#884444;border:1px solid rgba(255,26,26,0.02);border-radius:8px;cursor:pointer;font-size:14px;">
                        ЗАКРЫТЬ
                    </button>
                </div>
            </div>
