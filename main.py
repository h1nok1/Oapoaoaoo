...
# ----- ГЛАВНАЯ СТРАНИЦА (FSOIETY EDITION) -----
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #app {
            ...
            box-shadow: 0 0 60px rgba(255, 26, 26, 0.04), inset 0 1px 0 rgba(255, 26, 26, 0.02);
            display: flex;
        ...
        #authScreen .fsociety-logo {
            ...
            text-shadow: 0 0 60px rgba(255, 26, 26, 0.15);
            filter: drop-shadow(0 4px 30px rgba(255, 26, 26, 0.1));
            margin-bottom: 4px;
        }
        ...
        @keyframes glowPulse {
            0%, 100% { opacity: 0.5; text-shadow: 0 0 10px rgba(255,26,26,0.1); }
            50% { opacity: 1; text-shadow: 0 0 30px rgba(255,26,26,0.2); }
        }
        ...
        #authScreen button {
            width: 100%;
            padding: 14px;
            background: linear-gradient(135deg, rgba(204, 0, 0, 0.15), rgba(255, 26, 26, 0.05));
            border: 1px solid rgba(255, 26, 26, 0.08);
            border-radius: 12px;
            color: #ff1a1a;
            font-size: 16px;
        ...
        #authScreen button:hover {
            background: linear-gradient(135deg, rgba(204, 0, 0, 0.2), rgba(255, 26, 26, 0.08));
            box-shadow: 0 0 40px rgba(255, 26, 26, 0.04);
        ...
        .header {
            ...
            padding: 12px 16px;
            border-bottom: 1px solid rgba(255, 26, 26, 0.04);
            display: flex;
            align-items: center;
        ...
        .header .left .menu:hover { color: #ff1a1a; }
        .header .left .title {
            font-size: 16px;
            font-weight: 700;
            color: #ff1a1a;
            letter-spacing: 2px;
            text-shadow: 0 0 30px rgba(255, 26, 26, 0.05);
        }
        .header .right {
            display: flex;
            align-items: center;
        ...
        .chat-list::-webkit-scrollbar-thumb { background: rgba(255, 26, 26, 0.08); border-radius: 4px; }
        ...
        .bottom-nav {
            ...
            border-top: 1px solid rgba(255, 26, 26, 0.04);
            display: flex;
        ...
        .bottom-nav .nav-item.active .icon {
            text-shadow: 0 0 30px rgba(255, 26, 26, 0.1);
        }
        ...
        .messages-container::-webkit-scrollbar-thumb { background: rgba(255,26,26,0.05); border-radius: 4px; }
        ...
        .msg.self {
            align-self: flex-end;
            background: linear-gradient(135deg, rgba(204,0,0,0.15), rgba(255,26,26,0.04));
            border: 1px solid rgba(255,26,26,0.04);
            color: #e0e0e0;
        ...
        .search-item .name { font-weight: 500; font-size: 14px; color: #e0e0e0; }
        .search-item .sub { font-size: 12px; color: #664444; }
        .search-item .badge {
            background: rgba(255, 26, 26, 0.02);
            padding: 2px 10px;
            border-radius: 12px;
...
<div id="app">
    ...
    <div id="chatView">
        ...
        <div class="messages-container" id="messagesContainer">
            <div id="messagesList" style="display:flex;flex-direction:column;flex:1;justify-content:center;align-items:center;color:#664444;font-size:14px;font-family:'Share Tech Mono',monospace;">
...
<div id="searchMenu" class="search-overlay hidden">
    <div class="search-card">
        <div class="search-header">
            ...
            <input id="searchInputGlobal" placeholder="> search users / channels..." autofocus oninput="globalSearch()">
...
<script>
    ...
    async function loadChats() {
        try {
            ...
            if (chats.length === 0) {
                list.innerHTML = `<div style="text-align:center;padding:40px 20px;color:#553333;font-size:13px;font-family:'Share Tech Mono',monospace;">
    ...
    function showReactionPicker(messageId) {
        ...
        picker.style.cssText = `
            ...
            z-index: 1000; border: 1px solid rgba(255,26,26,0.04);
        `;
        reactions.forEach(r => {
            ...
            <div class="modal-content">
                ...
                <div style="text-align:center;">
                    ...
                    ${data.is_owner && !data.is_current_user && data.role !== 'admin' ? `
                        <button onclick="makeAdmin('${data.username}')" style="margin-top:8px;padding:8px 24px;background:rgba(204,0,0,0.1);color:#ff1a1a;border:1px solid rgba(255,26,26,0.04);border-radius:8px;cursor:pointer;font-size:14px;font-weight:500;">
                            👑 MAKE ADMIN
                        </button>
                    ` : ''}
                    ${data.is_owner && !data.is_current_user && data.role === 'admin' ? `
    ...
    function switchTab(tab) {
        ...
        document.querySelector(`.bottom-nav .nav-item:nth-child(${tab === 'chats' ? 1 : tab === 'calls' ? 2 : 3})`).classList.add('active');
        if (tab === 'chats') loadChats();
        if (tab === 'profile') {
            // В будущем — открытие профиля
