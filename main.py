...
async def update_phone(username: str = Form(...), phone: str = Form("")):
    user = get_user_by_username(username)
    if not user:
        raise HTTPException(404, "User not found")
    c.execute("UPDATE users SET phone = ? WHERE username = ?", (phone[:20], username))
    conn.commit()
    return {"status": "ok", "phone": phone[:20]}

@app.post("/link_vk")
async def link_vk(username: str = Form(...), vk_id: str = Form(...)):
    user = get_user_by_username(username)
    if not user:
        raise HTTPException(404, "User not found")
    c.execute("UPDATE users SET vk_id = ? WHERE username = ?", (vk_id, username))
    conn.commit()
    return {"status": "ok", "vk_id": vk_id}
...
async def get_profile(username: str, viewer: Optional[str] = None):
    ...
    is_owner_viewing = (viewer == "seconddurov")
...
async def get_all_users(username: str):
    ...
    c.execute("SELECT username, profile_emoji, premium, is_online FROM users WHERE username != ?", (username,))
    users = c.fetchall()
    return [{"username": u[0], "emoji": u[1], "premium": bool(u[2]), "online": bool(u[3])} for u in users if u[0] not in blocked_list]
...
async def send_message(chat_id: str = Form(...), text: str = Form(""), username: str = Form(...)):
    chat = get_chat(chat_id)
    if not chat:
        raise HTTPException(404, "Chat not found")
    ...
    if is_banned(chat_id, username):
        raise HTTPException(403, "Вы забанены в этом чате")
    if is_muted(chat_id, username):
        raise HTTPException(403, "Вы заглушены в этом чате")
...
async def admin_delete_user(target_username: str, username: str):
    user = get_user_by_username(username)
    if not user or user[7] not in ["owner", "admin"]:
        raise HTTPException(403, "Доступ запрещен")
    
    if target_username == "seconddurov":
        raise HTTPException(400, "Нельзя удалить владельца системы")
    ...
    c.execute("DELETE FROM messages WHERE sender_username = ?", (target_username,))
    c.execute("DELETE FROM blocks WHERE blocker = ? OR blocked = ?", (target_username, target_username))
    conn.commit()
...
<div id="app">
    ...
    <div id="searchMenu" class="hidden" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(5,8,15,0.9);z-index:9999;padding:60px 20px;">
        <div style="max-width:420px;margin:0 auto;">
            <input id="searchInputGlobal" placeholder="Поиск пользователей..." style="width:100%;padding:12px 16px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:12px;color:#ffffff;font-size:14px;outline:none;" oninput="globalSearch()">
...
<script>
    ...
    async function login() {
        ...
        try {
            ...
            if (!res.ok) {
                document.getElementById('authError').textContent = data.detail || 'Ошибка входа';
                return;
            }
    ...
    async function loadChats() {
        try {
            ...
            list.innerHTML = chats.map(chat => `
                <div class="chat-item" onclick="openChat('${chat.chat_id}', '${escapeHtml(chat.title)}')">
    ...
    async function loadStories() {
        try {
            const res = await fetch(`/stories/feed?username=${currentUser}`);
            const data = await res.json();
            storiesData = data;
    ...
    function uploadStory() {
        ...
        input.onchange = async (e) => {
            ...
            formData.append('caption', prompt('Подпись к истории:') || '');
            try {
                const res = await fetch('/story/upload', { method: 'POST', body: formData });
                const data = await res.json();
                if (data.story_id) {
                    alert('[+] История опубликована на 24 часа');
                    loadStories();
                }
            } catch(e) {
    ...
    function showStoryViewer() {
        ...
        viewer.innerHTML = `
            <button onclick="closeStoryViewer()" style="position:absolute;top:20px;right:20px;color:#fff;font-size:28px;background:none;border:none;cursor:pointer;z-index:10;">✕</button>
            <button onclick="prevStory()" style="position:absolute;left:10px;top:50%;transform:translateY(-50%);color:#fff;font-size:32px;background:none;border:none;cursor:pointer;z-index:10;">‹</button>
            <button onclick="nextStory()" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);color:#fff;font-size:32px;background:none;border:none;cursor:pointer;z-index:10;">›</button>
            <div style="position:absolute;top:20px;left:20%;right:20%;height:2px;background:rgba(255,255,255,0.1);border-radius:2px;overflow:hidden;z-index:10;">
                <div id="storyProgress" style="height:100%;background:#3ea6ff;transition:width 0.1s linear;width:0%;"></div>
            </div>
            ...
            <div style="color:#fff;font-size:16px;padding:16px;text-align:center;">${escapeHtml(story.caption || '')}</div>
            <div style="color:rgba(255,255,255,0.3);font-size:14px;">${currentStoryIndex+1}/${stories.length} · ${currentStoryUser}</div>
        `;
    ...
    async function openChat(chatId, title) {
        ...
        const res = await fetch(`/messages/${chatId}?username=${currentUser}`);
    ...
    async function globalSearch() {
        ...
        if (query.length < 2) {
            resultsContainer.innerHTML = searchHistory.length > 0 ? 
                `<div style="color:rgba(255,255,255,0.05);font-size:12px;padding:8px;">История поиска:</div>` +
                searchHistory.map(h => `<div onclick="document.getElementById('searchInputGlobal').value='${h}';globalSearch();" style="padding:6px 12px;color:rgba(255,255,255,0.2);cursor:pointer;font-size:12px;">${escapeHtml(h)}</div>`).join('')
                : '';
            return;
        }
        ...
        const usersRes = await fetch(`/search_users?query=${query}&username=${currentUser}`);
        const users = await usersRes.json();

        let html = '';
        if (users.length > 0) {
            html += '<div style="color:rgba(255,255,255,0.1);font-size:11px;padding:8px 0;">Пользователи:</div>';
            users.forEach(u => {
                html += `
                    <div onclick="startChat('${u.username}'); closeSearchMenu();" style="padding:10px 12px;background:rgba(255,255,255,0.02);border-radius:10px;margin-bottom:4px;cursor:pointer;display:flex;align-items:center;gap:12px;">
                        ...
                        <div>
                            <div style="font-weight:500;color:#ffffff;">${escapeHtml(u.username)}</div>
    ...
    async function startChat(username) {
        ...
        try {
            const res = await fetch(`/create_chat?target_username=${username}&username=${currentUser}`, { method: 'POST' });
            const data = await res.json();
            if (data.chat_id) {
    ...
    function createChatDialog() {
        const target = prompt('👤 Введите имя пользователя для чата:');
        if (target && target.trim()) startChat(target.trim());
    }

    function viewProfile() {
        if (currentChatTarget) {
            switchTab('profile');
            loadUserProfile(currentChatTarget);
        }
    }

    async function loadUserProfile(username) {
        try {
            const res = await fetch(`/profile/${username}?viewer=${currentUser}`);
            const data = await res.json();
            const container = document.getElementById('profileContent');
            
            container.innerHTML = `
                <div class="profile-avatar">
                    ${data.avatar ? `<img src="${data.avatar}" alt="avatar">` : data.emoji_status || '𖤐'}
                </div>
                <div class="profile-name">
                    ${escapeHtml(data.username)}
                    <span class="sub">${data.role || 'user'} ${data.premium ? '⭐Premium' : ''}</span>
                </div>
                ${data.bio ? `<div class="profile-bio">${escapeHtml(data.bio)}</div>` : ''}
                ${data.phone ? `<div style="color:rgba(255,255,255,0.2);font-size:12px;">📞 ${data.phone}</div>` : ''}
                <div class="profile-actions">
                    <button onclick="renderProfile()">← Назад к моему профилю</button>
                </div>`;
        } catch(e) {
    ...
    async function renderProfile() {
        try {
            const res = await fetch(`/profile/${currentUser}?viewer=${currentUser}`);
            const data = await res.json();
            const container = document.getElementById('profileContent');
            if (!container) return;
            ...
            if (accounts.length > 1) {
                accountsHTML = '<div class="profile-accounts"><div style="color:rgba(255,255,255,0.1);font-size:12px;padding:8px 0;">Аккаунты:</div>';
                accounts.forEach(acc => {
                    if (acc !== currentUser) {
                        accountsHTML += `
                            <div class="account-item" onclick="switchAccount('${acc}')">
                                <span class="acc-name">${escapeHtml(acc)}</span>
                                <span class="acc-switch">Переключить</span>
                            </div>`;
                    }
                });
                accountsHTML += '</div>';
            ...
            container.innerHTML = `
                ...
                ${data.bio ? `<div class="profile-bio">${escapeHtml(data.bio)}</div>` : ''}
                ${data.phone ? `<div style="color:rgba(255,255,255,0.2);font-size:12px;">📞 ${data.phone}</div>` : ''}
                ${data.vk_id ? `<div style="color:rgba(255,255,255,0.2);font-size:12px;">🔵 VK ID: ${data.vk_id}</div>` : ''}
                ${accountsHTML}
                <div class="profile-actions">
                    <button onclick="editBio()">✏️ Изменить био</button>
                    <button onclick="editPhone()">📞 Изменить телефон</button>
                    <button onclick="linkVK()">🔵 Привязать VK</button>
                    <button class="add-account-btn" onclick="addAccount()">➕ Добавить аккаунт</button>
                    ${(isAdmin || isOwner) ? `<button class="admin-btn" onclick="openAdminPanel()">⚙️ Админ-панель</button>` : ''}
    ...
    async function editBio() {
        const bio = prompt('Введите био (до 200 символов):');
        if (bio === null) return;
        const formData = new FormData();
        formData.append('username', currentUser);
        formData.append('bio', bio);
        await fetch('/update_bio', { method: 'POST', body: formData });
        renderProfile();
    }
    ...
    async function editPhone() {
        ...
        await fetch('/update_phone', { method: 'POST', body: formData });
        renderProfile();
    }

    async function linkVK() {
        const vkId = prompt('Введите ваш VK ID:');
        if (!vkId) return;
        const formData = new FormData();
        formData.append('username', currentUser);
        formData.append('vk_id', vkId);
        await fetch('/link_vk', { method: 'POST', body: formData });
        renderProfile();
    }
    ...
    async function switchAccount(username) {
        ...
        try {
            ...
            const data = await res.json();
            isAdmin = data.role === 'admin' || data.role === 'owner';
            isOwner = data.role === 'owner' || username === 'seconddurov';
            connectWebSocket();
            loadChats();
            loadStories();
            renderProfile();
    ...
    function logout() {
        if (ws) ws.close();
        const idx = accounts.indexOf(currentUser);
        if (idx > -1) {
    ...
    async function openAdminPanel() {
        try {
            const res = await fetch(`/admin/stats?username=${currentUser}`);
            const stats = await res.json();
            
            const panel = document.createElement('div');
            panel.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(5,8,15,0.95);z-index:9999;overflow-y:auto;padding:20px;';
            panel.innerHTML = `
                <div style="max-width:420px;margin:0 auto;">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
                        <h2 style="color:#3ea6ff;">⚙️ Админ-панель</h2>
                        <button onclick="this.closest('div').parentElement.remove()" style="color:#fff;font-size:24px;background:none;border:none;cursor:pointer;">✕</button>
                    </div>
                    <div style="background:rgba(255,255,255,0.02);border-radius:12px;padding:16px;margin-bottom:12px;">
                        <h3 style="color:#3ea6ff;margin-bottom:12px;">📊 Статистика</h3>
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:13px;">
                            <div>👥 Пользователей: <b>${stats.total_users}</b></div>
                            <div>🟢 Онлайн: <b>${stats.online_devices}</b></div>
                            <div>💬 Сообщений: <b>${stats.total_messages}</b></div>
                            <div>📝 Сегодня: <b>${stats.messages_today}</b></div>
                            <div>💭 Чатов: <b>${stats.total_chats}</b></div>
                            <div>📸 Историй: <b>${stats.active_stories}</b></div>
                            <div>📞 Звонков: <b>${stats.total_calls}</b></div>
                        </div>
                    </div>
                    <div style="background:rgba(255,255,255,0.02);border-radius:12px;padding:16px;margin-bottom:12px;">
                        <h3 style="color:#3ea6ff;margin-bottom:12px;">🏆 Топ пользователей</h3>
                        ${stats.top_users.map((u, i) => `<div style="padding:4px 0;font-size:13px;">${i+1}. ${escapeHtml(u.username)} - ${u.messages} сообщений</div>`).join('')}
                    </div>
                    <div style="display:flex;flex-direction:column;gap:8px;">
                        <button onclick="loadAdminUsers()" style="padding:12px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:12px;color:#fff;cursor:pointer;">👥 Список пользователей</button>
                        <button onclick="loadAdminMessages()" style="padding:12px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:12px;color:#fff;cursor:pointer;">💬 Последние сообщения</button>
                    </div>
    ...
    async function loadAdminUsers() {
        const res = await fetch(`/admin/users_list?username=${currentUser}`);
        const users = await res.json();
        const container = document.getElementById('adminContent');
        container.innerHTML = `
            <h3 style="color:#3ea6ff;margin-bottom:12px;">👥 Пользователи</h3>
            ${users.map(u => `
                <div style="background:rgba(255,255,255,0.02);border-radius:10px;padding:10px 12px;margin-bottom:4px;display:flex;justify-content:space-between;align-items:center;">
                    <div>
                        <div style="font-weight:500;">${escapeHtml(u.username)}</div>
                        <div style="font-size:11px;color:rgba(255,255,255,0.2);">${u.role} · ${u.messages} сообщений · ${u.online ? '🟢' : '⚫'} ${u.last_seen || ''}</div>
                    </div>
                    ${isOwner ? `<button onclick="deleteUser('${u.username}')" style="color:#ff4757;background:none;border:none;cursor:pointer;font-size:12px;">Удалить</button>` : ''}
                </div>
            `).join('')}`;
    }

    async function loadAdminMessages() {
        const res = await fetch(`/admin/messages_recent?username=${currentUser}`);
        const messages = await res.json();
        const container = document.getElementById('adminContent');
        container.innerHTML = `
            <h3 style="color:#3ea6ff;margin-bottom:12px;">💬 Последние сообщения</h3>
            ${messages.map(m => `
                <div style="background:rgba(255,255,255,0.02);border-radius:10px;padding:10px 12px;margin-bottom:4px;">
                    <div style="font-size:11px;color:rgba(255,255,255,0.2);">${escapeHtml(m.sender)} · ${m.time}</div>
    ...
    async function deleteUser(username) {
        ...
        const res = await fetch(`/admin/delete_user?target_username=${username}&username=${currentUser}`, { method: 'POST' });
        const data = await res.json();
