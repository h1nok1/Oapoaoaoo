...
# ----- ГЛАВНАЯ СТРАНИЦА (СВЕТЛЫЙ РЕДИЗАЙН + БЕЛАЯ ОБВОДКА) -----
...
<head>
    ...
    <style>
        /* ============================================================
           СВЕТЛЫЙ РЕДИЗАЙН — DELTA (СИНИЙ НЕОН + БЕЛАЯ ОБВОДКА)
           ============================================================ */
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #app {
            ...
            box-shadow: 0 0 80px rgba(0, 102, 255, 0.02), inset 0 1px 0 rgba(255, 255, 255, 0.02);
            display: flex;
        ...
        @media (min-width: 480px) {
            #app { border-radius: 28px; height: 95vh; border: 1px solid rgba(255, 255, 255, 0.04); }
        }

        /* ============================================================
           ЭКРАН АВТОРИЗАЦИИ — СВЕТЛЫЙ
           ============================================================ */
        #authScreen {
            display: flex;
            flex-direction: column;
        ...
        .story-row::-webkit-scrollbar-thumb { background: rgba(255, 255, 255, 0.02); border-radius: 4px; }
        ...
        /* ----- КНОПКИ СОЗДАНИЯ (СВЕТЛЫЕ, БЕЛАЯ ОБВОДКА) ----- */
        .action-buttons {
            display: flex;
            gap: 8px;
            padding: 8px 18px;
        ...
        .chat-list::-webkit-scrollbar-thumb { background: rgba(255, 255, 255, 0.02); border-radius: 4px; }
        ...
        /* ----- ЭКРАН ЧАТА (без шара, с логотипом D) ----- */
        .empty-chat {
            display: flex;
            flex-direction: column;
            align-items: center;
        ...
        .empty-chat .logo-d-small {
            width: 48px;
            height: 48px;
        }

        .empty-chat .logo-d-small svg {
            width: 100%;
            height: 100%;
        }

        .empty-chat .logo-d-small svg .d-outline {
            fill: none;
            stroke: rgba(255, 255, 255, 0.04);
            stroke-width: 2;
        }

        /* ----- НИЖНЯЯ НАВИГАЦИЯ (СВЕТЛАЯ) ----- */
        .bottom-nav {
            background: rgba(8, 12, 24, 0.6);
        ...
        .search-item .name { font-weight: 500; font-size: 13px; color: rgba(255, 255, 255, 0.04); }
        .search-item .sub { font-size: 11px; color: rgba(255, 255, 255, 0.02); }
        .search-item .badge {
            background: rgba(255, 255, 255, 0.01);
            padding: 2px 10px;
            border-radius: 12px;
...
<div id="app">
    ...
    <div id="chatView">
        ...
        <div class="messages-container" id="messagesContainer">
            <div id="messagesList" class="empty-chat">
                <div class="logo-d-small">
                    <svg viewBox="0 0 100 100">
                        <path class="d-outline" d="M 20 20 L 80 20 Q 90 20 90 30 L 90 70 Q 90 80 80 80 L 20 80 Q 10 80 10 70 L 10 30 Q 10 20 20 20 Z" />
                        <line class="d-inner" x1="30" y1="20" x2="30" y2="80" />
                        <circle class="d-outline" cx="85" cy="50" r="3" />
        ...
        <div class="msg-input-area">
            <button onclick="document.getElementById('photoInput').click()" style="background:none;border:none;color:rgba(255,255,255,0.02);font-size:18px;cursor:pointer;">📷</button>
...
<script>
    ...
    async function loadChats() {
        try {
            ...
            if (chats.length === 0) {
                list.innerHTML = `<div style="text-align:center;padding:40px 20px;color:rgba(255,255,255,0.02);font-size:13px;font-family:'Share Tech Mono',monospace;">
    ...
    async function loadStories() {
        try {
            ...
            if (usernames.length === 0) {
                container.innerHTML = `<div style="color:rgba(255,255,255,0.02);font-size:12px;padding:4px 0;">Нет историй</div>`;
                return;
            }
    ...
    function showStoryViewer() {
        ...
        viewer.innerHTML = `
            ...
            <div style="color:rgba(255,255,255,0.02);font-size:14px;margin-top:8px;">${currentStoryIndex+1}/${stories.length}</div>
        `;
    ...
    function showReactionPicker(messageId) {
        ...
        picker.style.cssText = `
            ...
            z-index: 1000; border: 1px solid rgba(255,255,255,0.01);
        `;
        reactions.forEach(r => {
            ...
            btn.style.cssText = `
                ...
                transition: 0.2s;
            `;
            btn.onmouseover = () => { btn.style.background = 'rgba(255,255,255,0.02)'; };
            btn.onmouseout = () => { btn.style.background = 'rgba(255,255,255,0.01)'; };
            btn.onclick = () => {
    ...
    async function globalSearch() {
        ...
        if (!html) {
            html = `<div style="padding:20px;text-align:center;color:rgba(255,255,255,0.02);">> no results found</div>`;
        }
    ...
    function renderSearchHistory() {
        ...
        if (searchHistory.length === 0) {
            container.innerHTML = `<div style="padding:16px;text-align:center;color:rgba(255,255,255,0.02);font-size:13px;">> no search history</div>`;
            return;
        }
        container.innerHTML = `
    ...
    async function renderProfile() {
        ...
        container.innerHTML = `
            ...
            <div class="profile-accounts">
                <div style="font-size:12px;color:rgba(255,255,255,0.02);margin-bottom:6px;text-align:left;">Мои аккаунты</div>
                ${accounts.map(acc => `
            ...
            <div class="profile-actions">
                ...
                <button onclick="logout()" style="color:#ffffff;text-align:center;">🚪 Выйти</button>
            </div>
        `;
    }
    ...
    async function switchAccountInstant(username) {
        ...
        document.getElementById('profileContent').innerHTML = `<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.02);">🔄 Переключение...</div>`;
        ...
        modal.innerHTML = `
            <div class="modal-content">
                ...
                <div style="text-align:center;">
                    ...
                    <div style="font-size:14px;color:rgba(255,255,255,0.02);margin:4px 0;">
                    ...
                    ${data.bio ? `<div style="font-size:13px;color:rgba(255,255,255,0.02);margin:8px 0;padding:8px;background:rgba(255,255,255,0.01);border-radius:8px;">${escapeHtml(data.bio)}</div>` : ''}
                    ${canMakeAdmin ? `
                        <button onclick="makeAdmin('${data.username}')" style="margin-top:8px;padding:8px 24px;background:rgba(255,255,255,0.02);color:#ffffff;border:1px solid rgba(255,255,255,0.01);border-radius:8px;cursor:pointer;font-size:14px;font-weight:500;">
                            👑 Сделать администратором
                        </button>
                    ` : ''}
                    ${isOwner && data.role === 'admin' && title !== 'seconddurov' ? `
                        <button onclick="removeAdmin('${data.username}')" style="margin-top:8px;padding:8px 24px;background:rgba(255,255,255,0.01);color:rgba(255,255,255,0.01);border:1px solid rgba(255,255,255,0.01);border-radius:8px;cursor:pointer;font-size:14px;font-weight:500;">
    ...
    async function loadCallHistory() {
        ...
        if (calls.length === 0) {
            container.innerHTML = `<div style="text-align:center;padding:40px 20px;color:rgba(255,255,255,0.02);font-size:14px;">
                <span style="font-size:48px;display:block;margin-bottom:8px;opacity:0.1;">📞</span>
                > нет истории звонков
            </div>`;
        } else {
            container.innerHTML = `
                <div style="font-size:12px;color:rgba(255,255,255,0.02);margin-bottom:8px;">>> ИСТОРИЯ ЗВОНКОВ</div>
                ${calls.map(c => `
                    <div style="display:flex;justify-content:space-between;padding:8px 12px;background:rgba(255,255,255,0.01);border-radius:8px;margin-bottom:4px;border:1px solid rgba(255,255,255,0.01);">
                        <div>
                            <span style="font-weight:500;color:rgba(255,255,255,0.02);">${c.caller === currentUser ? '📤' : '📥'} ${c.caller === currentUser ? c.callee : c.caller}</span>
                            <span style="font-size:10px;color:rgba(255,255,255,0.01);display:block;">${c.status === 'completed' ? '✅' : '❌'} ${c.duration ? c.duration + 'с' : ''}</span>
                        </div>
                        <div style="font-size:10px;color:rgba(255,255,255,0.01);">${formatTime(c.time)}</div>
                    </div>
                `).join('')}
            `;
        }
    ...
    function renderSettings() {
        ...
        container.innerHTML = `
            <div style="font-size:12px;color:rgba(255,255,255,0.02);margin-bottom:8px;">>> НАСТРОЙКИ</div>
            <div style="display:flex;flex-direction:column;gap:6px;">
                <div style="display:flex;justify-content:space-between;padding:10px 12px;background:rgba(255,255,255,0.01);border-radius:8px;border:1px solid rgba(255,255,255,0.01);">
                    <span style="color:rgba(255,255,255,0.02);">🌐 Прокси</span>
                    <span style="color:rgba(255,255,255,0.01);">Отключён</span>
                </div>
                <div style="display:flex;justify-content:space-between;padding:10px 12px;background:rgba(255,255,255,0.01);border-radius:8px;border:1px solid rgba(255,255,255,0.01);">
                    <span style="color:rgba(255,255,255,0.02);">🔒 Шифрование</span>
                    <span style="color:#ffffff;">AES-256-GCM</span>
                </div>
                <div style="display:flex;justify-content:space-between;padding:10px 12px;background:rgba(255,255,255,0.01);border-radius:8px;border:1px solid rgba(255,255,255,0.01);">
                    <span style="color:rgba(255,255,255,0.02);">📱 Устройств</span>
                    <span style="color:rgba(255,255,255,0.01);">${deviceId}</span>
                </div>
                <div style="display:flex;justify-content:space-between;padding:10px 12px;background:rgba(255,255,255,0.01);border-radius:8px;border:1px solid rgba(255,255,255,0.01);">
                    <span style="color:rgba(255,255,255,0.02);">📦 Версия</span>
                    <span style="color:rgba(255,255,255,0.01);">1.0 Fsociety</span>
                </div>
                ${isAdmin ? `
                    <div style="display:flex;justify-content:space-between;padding:10px 12px;background:rgba(255,255,255,0.01);border-radius:8px;border:1px solid rgba(255,255,255,0.01);cursor:pointer;" onclick="loadAdminStats()">
                        <span style="color:#ffffff;">👑 Админ-панель</span>
